...
public class HurdleHitScript : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player") && check)
        {
            ...
            Vector3 backDirection = (other.transform.position - transform.position).normalized;
            backDirection.y = 0f;
            Vector3 targetPosition = other.transform.position + backDirection * 2f;
            // Optional: stop current tween on player
            other.transform.DOKill();
            // Disable movement while pushing back
            playerMovementController.enabled = false;
            other.transform
                .DOMove(targetPosition, 0.35f)
                .SetEase(Ease.Linear)
                .OnComplete(() =>
                {
            ...
            switch (hurdleKind)
            {
                case HurdleKind.Box:
                    ...
                    GameManager.Instance.SoundManager.Haptic_BoxHit();         // Add haptic feedback for box hit
                    break;
                case HurdleKind.Barrel:
                    GameManager.Instance.SoundManager.Play_BarrelHitSound(AS);
                    GameManager.Instance.SoundManager.Haptic_BarrelHit();
                    break;
                case HurdleKind.WoodenLog:
                    GameManager.Instance.SoundManager.Play_WoodenLogHitSound(AS);
                    GameManager.Instance.SoundManager.Haptic_WoodenLogHit();
                    break;
            }
